                ...
                <div class="card">
                            ...
                            <div class="">
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-merge input-group-custom width-500px">
                                        ...
                                        <input type="hidden" name="type" value="{{ request('category_type') }}">
                                    </div>
                                </form>
                            </div>
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ url()->current() }}?category_type=store{{ request('searchValue') ? '&searchValue='.request('searchValue') : '' }}" type="button" class="btn {{ request('category_type') == 'store' || !request('category_type') ? 'btn--primary' : 'btn-outline--primary' }} text-nowrap">
                                    {{translate('shops')}}
                                </a>
                                <a href="{{ url()->current() }}?category_type=restaurant{{ request('searchValue') ? '&searchValue='.request('searchValue') : '' }}" type="button" class="btn {{ request('category_type') == 'restaurant' ? 'btn--primary' : 'btn-outline--primary' }} text-nowrap">
                                    {{translate('restaurants')}}
                                </a>
                                <div class="dropdown">
                                    <a type="button" class="btn btn-outline--primary text-nowrap btn-block" href="{{route('admin.vendors.export',['searchValue' => request('searchValue'), 'type' => request('type')])}}">
                    ...
                    <div class="table-responsive">
                        <table
                            ...
                            <tr>
                                <th>{{translate('SL')}}</th>
                                <th>{{ request('type') == 'restaurant' ? translate('restaurant_name') : translate('shop_name') }}</th>
