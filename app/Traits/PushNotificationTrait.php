   ...
   protected function sendOrderNotification(string $key, string $type, object|array $order, object|array $data = []): void
    {
        try {
            ...
            if ($type == 'customer') {
                ...
                if ($value) {
                    $value = $this->textVariableDataFormat(
                        ...
                        deliveryManName: "{$order->deliveryMan?->f_name} {$order->deliveryMan?->l_name}",
                    ...
                    if ($phone) {
                        $notificationService->sendWhatsAppNotification($phone, $postData);
                    }
                }
            }
            ...
            if ($type == 'seller') {
                ...
                if ($value_seller) {
                    $value_seller = $this->textVariableDataFormat(
                        value: $value_seller,
                        key: $key,
                        userName: "{$order->customer?->f_name} {$order->customer?->l_name}",
                        shopName: $order->seller?->shop?->name,
            ...
            if ($type == 'delivery_man') {
                ...
                if ($value_delivery_man) {
                    $value_delivery_man = $this->textVariableDataFormat(
                        value: $value_delivery_man,
                        key: $key,
                        userName: "{$order->customer?->f_name} {$order->customer?->l_name}",
                        shopName: $order->seller?->shop?->name,
                        deliveryManName: "{$order->deliveryMan?->f_name} {$order->deliveryMan?->l_name}",
            ...
            info('Error in sendOrderNotification: ' . $e->getMessage());
        }
    }
