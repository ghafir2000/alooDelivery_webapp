...
class SMSModule
{
    ...
    {
        $config = self::get_settings('whatsapp'); 
        if (isset($config) && $config['status'] == 1) {
            return self::whatsapp($receiver, $otp);
        }
     ...
     public static function whatsapp($receiver, $otp)
    {
        // We use our new helper function (created in Step 2) to get the settings
        $config = self::get_settings('whatsapp'); 
        $response = 'error';
        ...
        if (isset($config) && $config['status'] == 1) {
            try {
                // Construct the message from the OTP template stored in your settings
                $message = str_replace("#OTP#", $otp, $config['otp_template']);
                $postData = ['description' => $message];

                // Use the NotificationService to send the WhatsApp message
                $notificationService = new \App\Services\NotificationService();
                $notificationService->sendWhatsAppNotification($receiver, $postData);
                
                $response = 'success';

            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::error('WhatsApp OTP Sending Failed: ' . $e->getMessage());
                $response = 'error';
            }
        }
        return $response;
