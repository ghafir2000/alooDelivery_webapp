   ...
   public static function getBestSellingProductsList($request,  $limit = 10, $offset = 1)
{
    ...
    $productListData = Product::active()->withSum('orderDetails', 'qty', function ($query) {
        ...
        if (in_array($productType, ['restaurant', 'store'])) {
        $productListData = $productListData->where('type', $productType);
    }
   ...
   public static function get_discounted_product($request, $limit = 10, $offset = 1)
{
    ...
    $paginator = Product::with(['rating', 'reviews', 'tags', 'clearanceSale' => function ($query) {
        ...
        ->when($request->has('discount') && is_numeric($request->discount), function ($query) use ($request) {
            return $query->where('discount', '=', $request->discount);
        })
        ->when($request->has('category_type') && in_array($request->category_type, ['restaurant', 'store']), function ($query) use ($request) {
