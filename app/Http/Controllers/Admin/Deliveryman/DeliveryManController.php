...
class DeliveryManController extends Controller
{
    ...
    {
        return view(DeliveryMan::TRACKING[VIEW]);
    }
...
public function showDriverTracking()
    {
        $locations = DeliveryManLocation::query()->get();
        دLog::info('Delivery locations fetched:', ['count' => $locations->count(), 'data' => $locations->toArray()]);
        if ($locations->isEmpty()) {
            \Log::warning('No delivery locations found in the database.');
        }
        return view('admin-views.delivery-man.tracking', compact('locations'));
    }
