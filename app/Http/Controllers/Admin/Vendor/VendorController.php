...
class VendorController extends BaseController
{
    ...
    public function getListView(Request $request): View
    {
        ...
        if ($request->has('category_type') && in_array($request->input('category_type'), ['store', 'restaurant'])) {
            ...
            $vendors = $vendors->filter(function ($vendor) use ($type) {
                return $vendor->shop && $vendor->shop->type === $type;
            })->values();

            $vendors = new \Illuminate\Pagination\LengthAwarePaginator(
                $vendors,
                $vendors->count(),
                getWebConfig(name: WebConfigKey::PAGINATION_LIMIT),
     ...
     public function categoryList()
{
    ...
    return view('admin-views.vendor.category_list', compact('categories'));
}
