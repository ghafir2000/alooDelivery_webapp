...
class DeliveryManController extends Controller
{
    ...
    public function update_delivery_man_location(Request $request)
    {
        // Get token from Authorization header
        $token = $request->bearerToken();

        // Find delivery man by token
        $deliveryMan = DeliveryMan::where('auth_token', $token)
            ->where('is_active', 1)
            ->first();
        ...
        $locations = DeliveryManLocation::query()->updateOrCreate(
            [
...
{
    $locations = DeliveryManLocation::query()
        ->with(['delivery_man:id,f_name,l_name', 'delivery_man.current_orders.billingAddress'])
        ->get();

    $formattedLocations = $locations->map(function ($location) {
        return [
            'f_name' => $location->delivery_man->f_name,
            'l_name' => $location->delivery_man->l_name,
            'delivery' => [
                'latitude' => $location->latitude,
                'longitude' => $location->longitude
            ],
            'billing' => $location->delivery_man->current_orders && $location->delivery_man->current_orders->billingAddress
                ? [
                    'latitude' => $location->delivery_man->current_orders->billingAddress->latitude,
                    'longitude' => $location->delivery_man->current_orders->billingAddress->longitude
                ]
                : null
        ];
    })->values();

    \Log::info('API: Delivery locations fetched:', [
        'count' => $formattedLocations->count(),
        'data' => $formattedLocations->toArray()
    ]);

    if ($formattedLocations->isEmpty()) {
        \Log::warning('API: No valid delivery locations found in the database.');
    }
