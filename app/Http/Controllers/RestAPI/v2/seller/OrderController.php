...
class OrderController extends Controller
{
    ...
    public function update_payment_status(Request $request)
    {
     ...
     public function getDeliveryPricePerKm(Request $request)
    {
        try {
            $pricePerKm = DeliverySetting::where('key', 'cost_per_km')->value('value') ?? 0;
            \Log::info('API: Delivery price per km fetched:', ['price' => $pricePerKm]);
            ...
            \Log::error('API: Failed to fetch delivery price per km:', ['error' => $e->getMessage()]);
            return response()->json([
                'status' => 'error',
                'message' => 'Failed to fetch price per km',
            ], 500);
        }
    }

    public function updateDeliveryPricePerKm(Request $request)
    {
        try {
            $request->validate([
            ...
            $pricePerKm = $request->input('price_per_km');
            DeliverySetting::updateOrCreate(
                ['key' => 'cost_per_km'],
                ['value' => $pricePerKm, 'updated_at' => now()]
            );

            \Log::info('API: Delivery price per km updated:', ['price' => $pricePerKm]);
           ...
           \Log::error('API: Failed to update delivery price per km:', ['error' => $e->getMessage()]);
            return response()->json([
                'status' => 'error',
