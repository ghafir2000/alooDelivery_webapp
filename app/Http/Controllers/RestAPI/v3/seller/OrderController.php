...
class OrderController extends Controller
{
    ...
    public function createExternalOrder(Request $request)
    {

        $validator = Validator::make($request->all(), [
            'order_data' => 'required|array',
            'order_data.longitude' => 'required|numeric',
            'order_data.latitude' => 'required|numeric',
            'order_data.customer_name' => 'required|string|max:255',
            'order_data.customer_address' => 'required|string',
            'external_shop_name' => 'required|string|max:255',
        ...
        $externalShopName = $request->input('external_shop_name');
        $apiKey = $request->input('api_key');
        $uid = $request->input('uid');


        $externalShop = ExternalShop::where('name', $externalShopName)
            ->where('uid', $uid)
            ->where('api_key', $apiKey)
            ->first();

        if (!$externalShop) {
            return response()->json(['message' => 'بيانات المتجر الخارجي غير صحيحة أو غير موجودة'], 403);
        }


        $billingAddress = new BillingAddress();

        $billingAddress->longitude = $orderData['longitude'];
        $billingAddress->latitude = $orderData['latitude'];
        $billingAddress->contact_person_name = $orderData['customer_name'];
        $billingAddress->address = $orderData['customer_address'];
        ...
        $order->shipping_cost = ($pricePerKm * 1.5) * $request->distance;
        ...
        return response()->json([
            ...
            'order_id' => $order->id,
            'cost' => ($pricePerKm * 1.5) * $request->distance,
