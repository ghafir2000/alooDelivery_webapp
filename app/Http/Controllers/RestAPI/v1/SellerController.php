 ...
 public function getSellerList(Request $request, $type)
{
    ...
    // Filter by shop name (query)
    if ($request->has('query') && !empty($request->input('query'))) {
        $searchQuery = $request->input('query');
        $sellers = $sellers->whereHas('shop', function ($query) use ($searchQuery) {
    ...
    $sellers = $sellers->get()
        ->each(function ($seller) {
            ...
            $vacationStartDate = $seller->shop ? date('Y-m-d', strtotime($seller->shop->vacation_start_date ?? '1970-01-01')) : '1970-01-01';
            $vacationEndDate = $seller->shop ? date('Y-m-d', strtotime($seller->shop->vacation_end_date ?? '1970-01-01')) : '1970-01-01';
    ...
    if ($request->has('category_type') && in_array($request->input('category_type'), ['store', 'restaurant'])) {
        $categoryType = $request->input('category_type');
        $sellers = $sellers->filter(function ($seller) use ($categoryType) {
            return $seller->shop && $seller->shop->type === $categoryType;
        })->values();
    }
    ...
    if ($request->has('category_id') && is_numeric($request->input('category_id'))) {
        ...
        $sellers = $sellers->filter(function ($seller) use ($categoryId) {
            return $seller->shop && $seller->shop->category_id == $categoryId;
        })->values();
    }
