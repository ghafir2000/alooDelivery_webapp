  ...
  public function calculateDistance(Request $request)
    {
        ...
        $pricePerKm = DeliverySetting::where('key', 'cost_per_km')->value('value') ?? 0;

        try {

            $validated = $request->validate([
                'sellerIds' => 'required|array|min:1',
                'sellerIds.*' => 'integer|exists:sellers,id',
                'address_id' => 'required|integer|exists:shipping_addresses,id',
            ]);

            // Get customer coordinates
            $customerCoords = $this->getCustomerCoordinates($validated['address_id']);
            if (!$customerCoords) {
                throw new \Exception('Invalid customer address');
            }

            // Get sellers' coordinates
            $sellersCoords = $this->getSellersCoordinates($validated['sellerIds']);
            if (empty($sellersCoords)) {
                throw new \Exception('No valid sellers found');
            }

            // Sort sellers by distance from customer (farthest to nearest)
            $sortedSellers = $this->sortSellersByDistance($sellersCoords, $customerCoords);

            // Calculate total distance
            $totalDistance = $this->calculatePathDistance($sortedSellers, $customerCoords);

            // Log the request
            \Log::info('API: Total distance calculated:', [
                'customer' => $customerCoords,
                'sellers' => $sortedSellers,
                'total_distance_km' => $totalDistance,
                'total_price' => $pricePerKm,
            ...
            return response()->json([
                ...
                'total_price' => $pricePerKm * round($totalDistance, 2),
                'path' => array_merge($sortedSellers, [$customerCoords]),
            ]);
        } catch (\Illuminate\Validation\ValidationException $e) {
            \Log::error('API: Invalid input for distance calculation:', ['errors' => $e->errors()]);
            return response()->json([
                'status' => 'error',
            ...
            \Log::error('API: Failed to calculate total distance:', ['error' => $e->getMessage()]);
            return response()->json([
                'status' => 'error',
                'message' => 'فشل في حساب المسافة الإجمالية',
            ], 500);
        }
    }

    /**
     * Get customer coordinates from ShippingAddress model.
     */
    private function getCustomerCoordinates($addressId)
    {
    ...
    /**
     * Get sellers' coordinates from Seller model.
     */
    private function getSellersCoordinates($sellerIds)
    {
        $sellers = Shop::whereIn('id', $sellerIds)->get();
        $coords = [];
        foreach ($sellers as $seller) {
            if (!is_null($seller->latitude) && !is_null($seller->longitude)) {
                $coords[] = [
                    'id' => $seller->id,
     ...
     * Sort sellers by distance from customer (farthest to nearest).
     */
    private function sortSellersByDistance($sellers, $customer)
    {
        usort($sellers, function ($a, $b) use ($customer) {
            $distanceA = $this->calculateDistanceBetweenPoints($a, $customer);
            $distanceB = $this->calculateDistanceBetweenPoints($b, $customer);
    ...
    /**
     * Calculate total distance from farthest seller to nearest to customer.
     */
    private function calculatePathDistance($sortedSellers, $customer)
    {
        $totalDistance = 0;

        // Calculate distances between consecutive sellers (farthest to nearest)
        for ($i = 0; $i < count($sortedSellers) - 1; $i++) {
            $totalDistance += $this->calculateDistanceBetweenPoints($sortedSellers[$i], $sortedSellers[$i + 1]);
        }

        // Add distance from the nearest seller to the customer
        if (!empty($sortedSellers)) {
            $totalDistance += $this->calculateDistanceBetweenPoints(end($sortedSellers), $customer);
        }
